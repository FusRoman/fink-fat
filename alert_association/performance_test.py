...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        exit(0)
            ...
            for i in range(1, len(all_night)):
                ...
                if verbose:  # pragma: no cover
                    ...
                    print("nb most recent traj: {}".format(len(np.unique(most_recent_traj['trajectory_id']))))
        ...
        mpc_plot = df_sso.groupby(["ssnamenr"]).agg({"ra": list, "dec": list, "candid":len}).reset_index()
        print("number of known trajectory in this dataset: {}".format(len(mpc_plot)))
        print("number of MPC trajectories that have a number of point greather strictly than 10: {}".format(len(mpc_plot[mpc_plot['candid']>10])))


        traj_df = pq.read_table('trajectory_output.parquet').to_pandas()

        print("number of detected trajectories: {}".format(len(np.unique(traj_df['trajectory_id']))))
        ...
        print("number of trajectories that have a number of point greather strictly than 10: {}".format(len(gb_traj)))
        ...
        fig, ax = plt.subplots(1, 1, figsize=(15, 15))
        ax.set_title("Ratio between the most common ssnamenr tag and the other ssnamenr tags")
