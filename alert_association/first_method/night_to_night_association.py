...
def angle_df(x):
    ...
    a = np.array([ra[0], dec[0]])
    b = np.array([ra[1], dec[1]])

    jd1 = jd[1]

    for i in range(2, len(ra)):
        
        c = np.array([ra[i], dec[i]])
        jd2 = jd[i]

        diff_jd = jd2 - jd1

        if diff_jd > 1:
            all_angle.append(angle_three_point(a, b, c) / diff_jd)
        else:
            all_angle.append(angle_three_point(a, b, c))
