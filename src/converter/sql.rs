...
fn create_tables(transaction: &mut postgres::Transaction<'_>) -> Result<(), postgres::Error> {
    transaction.batch_execute(
        ...
        -- write_sql_tables and given no FK into branches/observations: those
        -- tables are TRUNCATEd (with CASCADE) on every `convert` re-run, and a
        -- FK here would wipe this fit history along with them. observation_ids
        -- is a plain array rather than a join table for the same reason.
