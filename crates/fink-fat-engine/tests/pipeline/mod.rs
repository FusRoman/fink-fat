...
use photom::observation_dataset::{ObsDataset, iter::MemLayoutObservations};
...
use crate::synthetic_alerts::{
    SyntheticAlert, SyntheticDataset, TrajectoryTruth, write_and_load_parquet,
};
...
/// resolving its seed nodes through the seed store.
///
/// Since `seed.members` already contains `ObsId` values (= `dia_source_id`),
/// no alert store lookup is required.
pub(crate) fn track_dia_source_ids(
    ...
    for &seed_key in &track.nodes {
        if let Some(seed) = seed_store.try_get_seed(seed_key) {
...
pub(crate) fn match_truth_to_hypotheses(
    ...
    let hyp_sets: HashMap<u32, HashSet<u64>> = hypotheses
        .iter()
        .map(|(&hid, track)| (hid, track_dia_source_ids(track, seed_store)))
        .collect();
...
/// Build a plan whose `inputs` field carries a real per-night `ObsDataset`
/// and a runner plan whose inputs is empty (the runner only reads `stages`).
///
/// Returns `(data_plan, runner)` where `data_plan` should be passed as
/// `&mut plan` to `PipelineContext` and `runner` executes the stages.
pub(crate) fn make_plan_and_runner(
    stages: &[PipelineStage],
    persist: fink_fat_engine::engine_config::pipeline_policy::PersistPolicy,
    new_observation_dataset: ObsDataset,
) -> (PipelinePlan, PipelineRunner) {
    let plan = PipelinePlan {
        stages: stages.to_vec(),
        persist: persist.clone(),
        inputs: PipelineInputs {
            new_observation_dataset,
        },
    };
    let runner = PipelineRunner {
        plan: PipelinePlan {
            ...
            persist,
            inputs: PipelineInputs {
                new_observation_dataset: ObsDataset::empty(),
            },
        },
    };
    (plan, runner)
...
/// Collect all `dia_source_id`s present in the observation dataset.
...
/// Collect night IDs from the observation dataset (sorted).
pub(crate) fn collect_night_ids(state: &RuntimeState) -> Vec<NightId> {
    let mut nights: Vec<NightId> = state
        .obs_dataset
        .iter_night_id()
        .into_iter()
        .flatten()
...
/// Check if a given `dia_source_id` (ObsId) is present in the observation dataset.
pub(crate) fn obs_contains_id(state: &RuntimeState, id: u64) -> bool {
    state.obs_dataset.iter_observations().any(|o| *o.id() == id)
}

/// Count observations in the runtime state dataset.
pub(crate) fn count_observations(state: &RuntimeState) -> usize {
    state.obs_dataset.observation_count()
}

/// Count nights in the runtime state dataset.
pub(crate) fn count_nights(state: &RuntimeState) -> usize {
    state.obs_dataset.nb_night().unwrap_or(0)
}

/// Return observations for a specific night, sorted by `mjd_tt`.
///
/// Panics if the night is not present in the dataset.
pub(crate) fn get_night_observations_sorted<'a>(
    state: &'a RuntimeState,
    nid: &NightId,
) -> MemLayoutObservations<'a> {
    state
        .obs_dataset
        .materialize_night(nid)
        .unwrap_or_else(|| panic!("night {nid:?} not found in obs_dataset"))
}
...
pub(crate) fn run_pipeline(
    ...
    run_pipeline_with(
        ...
        max_gap_nights,
        &solver_manager,
        fink_fat_engine::engine_config::pipeline_policy::PersistPolicy::None,
    )
}
...
pub(crate) fn run_pipeline_with(
    ...
    for (run_idx, &nid) in night_ids.iter().enumerate() {
        let night_alerts: Vec<&SyntheticAlert> = dataset
            .alerts()
            .iter()
        ...
        let (mut plan, runner) = make_plan_and_runner(stages, persist.clone(), night_obs);
        let hooks = NoopHooks;
