...
def kalman_association(
    ...
    1                                            []
    2                                            []
    3                         [0.07941804624118987]
    4    [0.06932175782469661, 0.12832509809847767]
    dtype: object
    ...
    if len(jd_unique) > 2:
        min_jd = np.min(jd_unique)
        max_jd = np.max(jd_unique)
        if max_jd - min_jd <= 30 / 60 / 24:
            # if diff time between min and max are below 30 minutes
            # make predictions only for the min and max
            jd_unique = np.array([min_jd, max_jd])
        else:
            # otherwise, make 15 predictions between min and max for each kalman
            jd_unique = np.linspace(min_jd, max_jd, 15)
    ...
    ra_kal_pred = kal_pred["ra"].values
    dec_kal_pred = kal_pred["dec"].values
    ...
    result_pdf_kalman = results_pdf.merge(kalman_pdf, on="trajectory_id")
    ...
    # compute the separation between the kalman prediction and the filtered associations
    with pd.option_context("mode.chained_assignment", None):
        result_filtered["trajectory_id"] = result_filtered["trajectory_id"].astype(str)

        coord_alert_filtered = SkyCoord(
            result_filtered["ra"].values,
            result_filtered["dec"].values,
            unit="deg",
        )
        kalman_pred_filtered = SkyCoord(
            result_filtered["ra_pred"].values,
            result_filtered["dec_pred"].values,
            unit="deg",
        )
        result_filtered["sep"] = coord_alert_filtered.separation(
            kalman_pred_filtered
        ).deg

    # keep the minimum separation
    filtered_group = (
        result_filtered.groupby(["idx_alert", "trajectory_id"])
        .agg(sep_min=("sep", "min"))
        .reset_index()
        .groupby("idx_alert")
        .agg(list)
    ...
    estimator_id[idx_to_update] = filtered_group["trajectory_id"]
    ffdistnr[idx_to_update] = filtered_group["sep_min"]
